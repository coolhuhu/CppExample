...
class timestamp_t {
 ...
 private:
  ...
  std::string format(const char *fmt, ...) const {
    ...
    if (r < 0) return {};
    const size_t len = r;
    if (len < sizeof(buf)) return std::string(buf, len);
#if __cplusplus >= 201703L
    std::string s(len, '\0');
    va_start(args, fmt);
 ...
 private:
  ...
  void predict(const std::vector<float> &data_chunk) {
    ...
    if (speech_prob >= threshold) {
      ...
      std::copy(new_data.end() - context_samples, new_data.end(),
                _context.begin());
      return;
    }
  ...
  void process(const std::vector<float> &input_wav) {
    ...
    for (size_t j = 0; j < static_cast<size_t>(audio_length_samples);
      ...
      if (j + static_cast<size_t>(window_size_samples) >
        ...
        break;
      std::vector<float> chunk(&input_wav[j],
                               &input_wav[j] + window_size_samples);
      predict(chunk);
    }
...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < stamps.size(); i++) {
    ...
    float end_sec =
        std::rint((stamps[i].end / sample_rate_float) * 10.0f) / 10.0f;
    std::cout << "Speech detected from " << std::fixed << std::setprecision(1)
              << start_sec << " s to " << std::fixed << std::setprecision(1)
              << end_sec << " s" << std::endl;
  }
