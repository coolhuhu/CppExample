...
//     size_t res = fread(&global_header[0], sizeof(char), global_header_size,
//     fp); if (res != global_header_size) {
...
//       global_header_offset;  // relative offset of local file header, since
//       it
...
//   footer += (uint16_t)(nrecs + 1);           // number of records on this
//   disk footer += (uint16_t)(nrecs + 1);           // total number of records
...
// template <typename T>
// void npz_save(std::string zipname, std::string fname, const std::vector<T>
// data,
//               std::string mode = "w") {
//   std::vector<size_t> shape;
//   shape.push_back(data.size());
